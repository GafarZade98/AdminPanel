...
$(document).ready(function() {
    ...
    $('.product_carousel,.homeproduct_carousel,.sideproduct_carousel,.producttab_carousel,.brand_carousel, .blog-carousel,#ttmtestimonial_carousel,#ttmcat_carousel,.instagram_carousel, .gallery_album_carousel, .special_product_carousel').addClass('owl-carousel');
...
function specialcountdown() {
	$('.ttmtimer').each(function(){
		$(this).countdown($(this).attr('data-date'), function(event) {
			$(this).html(
				'<div class="clocktime days"><span class="clocktime-number">'+event.strftime('%D')+'</span><span class="clocktime-text">days</span></div><div class="clocktime hour"><span class="clocktime-number">'+event.strftime('%H')+'</span><span class="clocktime-text">hours</span></div><div class="clocktime minute"><span class="clocktime-number">'+event.strftime('%M')+'</span><span class="clocktime-text">mins</span></div><div class="clocktime second"><span class="clocktime-number">'+event.strftime('%S')+'</span><span class="clocktime-text">secs</span></div>'
			);
		});
...
// progress-bar js start 
	...
	if ($(document).width() <= 979) {
		...
		$('.bottom_block .header-cart').appendTo('.header_main .container');
		$('.bottom_block .ttm_myaccount').appendTo('.header_main .container');
		$('.bottom_block .top_bar_wishlist').appendTo('.header_main .container');
		$('.bottom_block .top_bar_compare').appendTo('.header_main .container');
    }
...
if ($(document).width() >= 768) {
	var divHeight = $('#content .box.title-content .ttm-content-box').height(); 
	$('.product-box.title-content .heading-title').css('height', divHeight+'px');
}
}
$(window).on("load", function() {
	heightDependency();
