<div class="container-fluid pt-5">
    <div class="text-center mb-4">
        <h2 class="section-title px-5"><span class="px-2">@lang('website.general.top_products')</span></h2>
    </div>
                ...
                <div class="card product-item border-0 mb-4">
                    ...
                    <div class="card-body border-left border-right text-center p-0 pt-4 pb-3">
                        ...
                        <div class="d-flex justify-content-center">
                            <h6>{{$product->getAttribute('price')}}</h6><h6 class="text-muted ml-2"><del>{{$product->getAttribute('price')*1.10}}</del></h6>
                        </div>
                    </div>
                        ...
                        <a class="btn btn-sm text-dark p-0" href="{{ route('cart.create') }}"
                           onclick="event.preventDefault(); document.getElementById('add-to-cart').submit();">
                            ...
                            <form id="add-to-cart" action="{{ route('cart.create') }}" method="POST" class="d-none">
                                ...
                                @csrf
                            </form><i class="fas fa-shopping-cart text-primary mr-1"></i>@lang('website.general.add_to_cart')</a>
                    </div>
                </div>
            </div>
            @endforeach
